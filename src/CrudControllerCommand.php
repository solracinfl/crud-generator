...
class CrudControllerCommand extends GeneratorCommand
{
    ...
    protected function buildClass($name)
    {
        ...
        // replace to use singular instead of plural
        // $crudNamePlural = str_plural($crudName);
        // $crudNamePluralCap = str_plural($crudNameCap);

        $crudNamePlural = str_singular($crudName);
        $crudNamePluralCap = str_singular($crudNameCap);

        $crudNameSingular = str_singular($crudName);

        $child_of_facility = $this->option('child_of_facility');

        if($child_of_facility=="true")
            $facility_child = '$request[\'facility_id\'] = Helper::currentFacility();';
        else
            $facility_child = '';

        return $this->replaceFacilityChild($stub, $facility_child)->replaceNamespace($stub, $name)->replaceCrudName($stub, $crudName)->replaceCrudNameCap($stub, $crudNameCap)->replaceCrudNamePlural($stub, $crudNamePlural)->replaceCrudNamePluralCap($stub, $crudNamePluralCap)->replaceCrudNameSingular($stub, $crudNameSingular)->replaceClass($stub, $name);
    }


    protected function replaceFacilityChild(&$stub, $facilityChild)
    {
        $stub = str_replace(
            '{{FacilityChild}}', $facilityChild, $stub
        );
    ...
    protected function getOptions()
    {
        return [
            ...
            ['child_of_facility', null, InputOption::VALUE_REQUIRED, 'Is this a direct child of a facility.', null],
        ];
    }
